"""Monitor OpenClaw processes for suspicious activity."""
...
from ..config import MONITOR_POLL_INTERVAL_SECONDS, SUSPICIOUS_PROCESS_PATTERNS
...
class ProcessMonitor(BaseMonitor):
    """Tracks OpenClaw process trees and flags suspicious children."""
        ...
        for child in children:
            ...
            if child_name and child_name != "node" and not child_name.startswith("node"):
                self.report_finding(Finding(
                    ...
                    detail=f"PID {child.pid} ({child_name}) is a child of OpenClaw PID {proc.pid}.",
                ))
