"""Network forensics: snapshot connections for OpenClaw processes."""
...
def _is_openclaw_process(proc: psutil.Process) -> bool:
    """Check if a process is related to OpenClaw."""
    try:
        name = proc.name().lower()
        cmdline = " ".join(proc.cmdline()).lower()
    except (psutil.NoSuchProcess, psutil.AccessDenied):
        return False
    return "openclaw" in name or "openclaw" in cmdline
...
class NetworkForensicsSweep(BaseSweep):
    ...
    def run(self) -> ModuleResult:
        ...
        openclaw_pids: set[int] = set()
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            if _is_openclaw_process(proc):
                openclaw_pids.add(proc.pid)

        if not openclaw_pids:
            findings.append(Finding(
                module=self.name,
                severity=Severity.INFO,
                title="No OpenClaw processes found",
                detail="No running processes match 'openclaw'.",
            ))
            return ModuleResult(module_name=self.name, findings=findings)

        # Get connections for OpenClaw processes
