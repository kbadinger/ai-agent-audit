"""Docker container security sweep for OpenClaw containers."""
...
class DockerSecuritySweep(BaseSweep):
    ...
    def run(self) -> ModuleResult:
        ...
        # Find OpenClaw-related containers
        try:
        ...
        if not container_ids:
            findings.append(Finding(
                module=self.name,
                severity=Severity.INFO,
                title="No OpenClaw containers found",
                detail="No running Docker containers match 'openclaw'.",
            ))
            return ModuleResult(module_name=self.name, findings=findings)
