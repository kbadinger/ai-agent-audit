...
def generate_sbom() -> dict[str, Any]:
    """Generate a CycloneDX 1.5 SBOM for the active agent installation."""
    ...
    return {
        ...
        "metadata": {
            ...
            "component": {
                "type": "application",
                "name": ACTIVE_PROFILE.slug,
                "description": f"{ACTIVE_PROFILE.display_name} AI agent installation",
            },
        },
...
def _scan_skills(components: list[dict]) -> None:
    ...
    for skill_dir in sorted(OPENCLAW_SKILLS.iterdir()):
        ...
        component: dict[str, Any] = {
            ...
            "name": skill_dir.name,
            "group": f"{ACTIVE_PROFILE.slug}-skills",
        }
...
def _scan_extensions(components: list[dict]) -> None:
    ...
    for ext_dir in sorted(OPENCLAW_EXTENSIONS.iterdir()):
        ...
        component: dict[str, Any] = {
            ...
            "name": ext_dir.name,
            "group": f"{ACTIVE_PROFILE.slug}-extensions",
            "version": "unknown",
        }
