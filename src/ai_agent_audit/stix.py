...
def findings_to_stix(findings: list[dict]) -> dict[str, Any]:
    ...
    # Identity for ai-agent-audit
    identity_id = _stix_id("identity", "ai-agent-audit")
    objects.append({
        "type": "identity",
        ...
        "description": f"Security audit daemon for {ACTIVE_PROFILE.display_name} installations",
    })
    ...
    return {
        "type": "bundle",
        "id": _stix_id("bundle", f"ai-agent-audit-{now}"),
        "objects": objects,
    }
...
def ioc_to_stix() -> dict[str, Any]:
    ...
    identity_id = _stix_id("identity", "ai-agent-audit-ioc")
    objects.append({
        "type": "identity",
        "spec_version": _STIX_VERSION,
        "id": identity_id,
        "created": now,
        "modified": now,
        "name": "ai-agent-audit IOC database",
        "identity_class": "system",
    })
    ...
    return {
        "type": "bundle",
        "id": _stix_id("bundle", f"ai-agent-audit-ioc-{now}"),
        "objects": objects,
    }
...
def _finding_to_pattern(f: dict) -> str:
    ...
    return f"[x-{ACTIVE_PROFILE.slug}-finding:module = '{module}']"
