...
from ..config import ACTIVE_PROFILE, OPENCLAW_LOG_DIR, SECRET_PATTERNS
...
class LogForensicsSweep(BaseSweep):
    ...
    def run(self) -> ModuleResult:
        ...
        log_glob = f"{ACTIVE_PROFILE.slug}-*.log"
        log_files = sorted(OPENCLAW_LOG_DIR.glob(log_glob))
        if not log_files:
            findings.append(Finding(
                module=self.name,
                severity=Severity.INFO,
                title="No log files found",
                detail=f"No {log_glob} files in {OPENCLAW_LOG_DIR}.",
            ))
            return ModuleResult(module_name=self.name, findings=findings)
