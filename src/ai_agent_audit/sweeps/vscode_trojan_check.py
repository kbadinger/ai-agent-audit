...
# Patterns to match in VS Code extension directory names (per active agent profile)
_EXTENSION_PATTERNS = [p.lower() for p in ACTIVE_PROFILE.vscode_extension_patterns]
                ...
                for pattern in _EXTENSION_PATTERNS:
                    if pattern in name_lower:
                        findings.append(Finding(
                            ...
                            title=f"Fake {ACTIVE_PROFILE.display_name} VS Code extension: {entry.name}",
                            detail=(
                                f"{ACTIVE_PROFILE.display_name} has no official VS Code extension. "
